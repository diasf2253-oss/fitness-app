...
class TestWeeklyReview:
    def test_two_week_comparison(self, auth_client):
        today = date.today()
        ...
        auth_client.post("/api/health/weight",
                         json={"date": today.isoformat(), "weight_kg": 83.5})

        data = auth_client.get("/api/insights/weekly").json()
    ...
    def test_habits_and_scales_in_review(self, auth_client):
        t = auth_client.post("/api/trackers", json={"name": "Meditate", "kind": "habit"}).json()
        m = auth_client.post("/api/trackers", json={"name": "Mood", "kind": "scale"}).json()
        today = date.today()
        for offset in (0, 1):
            auth_client.post(f"/api/trackers/{t['id']}/log",
                             json={"date": (today - timedelta(days=offset)).isoformat(), "value_num": 1})
        auth_client.post(f"/api/trackers/{m['id']}/log",
                         json={"date": today.isoformat(), "value_num": 4})

        cur = auth_client.get("/api/insights/weekly").json()["current"]
        assert cur["habits"] == [{"name": "Meditate", "done": 2, "days": 7}]
        assert cur["scales"] == [{"name": "Mood", "avg": 4.0}]


class TestCorrelations:
    def seed_pair_days(self, client, n, steps_for_day):
        today = date.today()
        for i in range(n):
            ...
            client.post("/api/health/steps",
                        json={"date": d, "steps": steps_for_day(i)})

    def test_strong_correlation_detected(self, auth_client):
        # Steps move linearly with sleep → r ≈ 1 on the Sleep×Steps pair
        self.seed_pair_days(auth_client, 12, lambda i: 5000 + i * 500)
        data = auth_client.get("/api/insights/correlations").json()
    ...
    def test_min_n_threshold(self, auth_client):
        # Only 5 overlapping days → pair must not appear
        self.seed_pair_days(auth_client, 5, lambda i: 5000 + i * 500)
        data = auth_client.get("/api/insights/correlations").json()
        assert not any({p["label_a"], p["label_b"]} == {"Sleep (h)", "Steps"}
                       for p in data["pairs"])

    def test_training_split(self, auth_client):
        m = auth_client.post("/api/trackers", json={"name": "Mood", "kind": "scale"}).json()
        today = date.today()
        ...
        for offset in range(5):
            ...
            db.add(WorkoutSession(
                user_id=auth_client.test_user_id,
                name=f"S{offset}",
    ...
    def test_split_needs_both_sides(self, auth_client):
        m = auth_client.post("/api/trackers", json={"name": "Mood", "kind": "scale"}).json()
        today = date.today()
