...
- Keep the Pytest suite green. **Known gap**: only `tests/test_safety_net.py` has been migrated to the new session-cookie auth pattern (see `tests/conftest.py`'s docstring) — the other test files still use the old bearer-token pattern and need the same mechanical migration before the suite is green again.
